...
export class CodePipelineHelper extends Construct {
  ...
  /**
   * Bake steps indexed by unique action name, as generated by `newWaveBakeStep` and `newStageBakeStep`.
   */
  private readonly bakeSteps: Record<string, BakeStepProps>;
   ...
   *
   * @param id - must be unique across all bake steps
   */
  public newWaveBakeStep(id: string, props: BakeStepProps) {
    return this.addNewBakeStep(id, id, props);
  }
   ...
   * Creates a new `Step` with a unique `id` using given prefix.
   *
   * @param id - must be unique across all bake steps for given stage
   */
  public newStageBakeStep(stage: cdk.Stage, id: string, props: BakeStepProps) {
    return this.addNewBakeStep(`${stage.stageName}.${id}`, id, props);
  }

  private addNewBakeStep(bakeStepId: string, enforcerId: string, props: BakeStepProps) {
    if (bakeStepId in this.bakeSteps) {
      throw new Error(`There is already a bake step with ID "${bakeStepId}"`);
    }
    this.bakeSteps[bakeStepId] = props;

    return new pipelines.ManualApprovalStep(enforcerId, {
