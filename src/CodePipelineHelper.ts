...
export class CodePipelineHelper extends Construct {
  ...
  /**
   * Bake steps indexed by unique action name, as generated by `newBakeStep`.
   */
   ...
   */
  public newBakeStep(id: string, props: BakeStepProps) {
    if (id in this.bakeSteps) {
      throw new Error(`There is already a bake step with ID "${id}"`);
    }
    this.bakeSteps[id] = props;

    return new pipelines.ManualApprovalStep(id, {
      comment: `**DeploymentSafetyEnforcer**

* **Bake time**: ${props.bakeTime.toHumanString()}
* **Alarm**: ${props.rejectOnAlarm ? props.rejectOnAlarm.alarm.alarmName : 'N/A'}`,
    });
  }
